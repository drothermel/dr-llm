...
4. `uv run ty check`
5. `./scripts/run-tests-non-integration.sh`
...
`./scripts/run-tests-non-integration.sh` runs the non-integration suite with `pytest-xdist`.
...
1. Local quality gate runs `./scripts/run-tests-non-integration.sh` plus `./scripts/run-tests-local.sh`.
1. `.github/workflows/ci.yml`:
    - `quality-unit`: `ruff format --check`, `ruff check .`, `ty check`, `pytest -m "not integration" -n auto` for fast PR feedback.
