...
from dspy_rich_trace_extract_common import (
    ...
    DspyDatasetSplitEntry,
    DspyDatasetTaskSplitEntry,
    DspyRichTraceDatasetSplitManifest,
    by_dataset_dir_for,
    dspy_rich_attempts_path_for,
    dspy_rich_traces_dir_for,
    safe_path_part,
)

app = typer.Typer(
    help="Split DSPy rich trace attempts by dataset and task id."
)
...
def main(
    corpus_root: Annotated[
        ...
        typer.Argument(
            ...
            dir_okay=True,
            readable=True,
            help="DSPy corpus root containing dspy_rich_traces/.",
        ),
    ] = DEFAULT_DSPY_CORPUS_ROOT,
    input_path: Annotated[
        ...
        typer.Option(
            ...
            dir_okay=False,
            readable=True,
            help="DSPy rich trace Parquet path.",
        ),
    ] = None,
    ...
    parquet_path = input_path or dspy_rich_attempts_path_for(corpus_root)
    destination = output_dir or by_dataset_dir_for(
        dspy_rich_traces_dir_for(corpus_root)
    )
    destination.mkdir(parents=True, exist_ok=True)
    attempts = pd.read_parquet(parquet_path)
    validate_attempts(attempts, parquet_path)

    dataset_entries: list[DspyDatasetSplitEntry] = []
    for dataset, dataset_rows in attempts.groupby(
        ...
        task_entries: list[DspyDatasetTaskSplitEntry] = []
    ...
    manifest = DspyRichTraceDatasetSplitManifest(
