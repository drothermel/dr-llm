# Migration Guide: Direct Providers to dr-llm
...
1. Configure `DR_LLM_DATABASE_URL`
...
3. Run `dr-llm tool worker run ...` for queued brokered tools

## 4) Preserve compatibility
1. Keep benchmark/task scoring in consumer repo.
2. Keep only shared llm/storage/session runtime concerns in `dr-llm`.
...
3. run `dr-llm run benchmark` before cutover
