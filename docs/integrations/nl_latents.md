...
## Goal
Use `llm-pool` for model calls + recording while keeping latent/task semantics in `nl_latents`.
...
4. `llm-pool` stores canonical call/session/tool traces.
...
## Notes
1. Keep `nl_latents` scoring/ranking logic outside `llm-pool`.
