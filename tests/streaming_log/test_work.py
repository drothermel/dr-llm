...
def test_queued_work_message_from_payload_rejects_non_object() -> None:
    with pytest.raises(TypeError, match="JSON object"):
        QueuedWorkMessage.from_payload(b"[]")


def test_queued_work_message_from_payload_requires_request() -> None:
    with pytest.raises(ValueError, match="request"):
        QueuedWorkMessage.from_payload(b"{}")


def test_queued_work_message_from_payload_rejects_non_object_metadata() -> (
    None
):
    payload = {
        "request": _request().model_dump(mode="json"),
    ...
    with pytest.raises(TypeError, match="metadata"):
        QueuedWorkMessage.from_payload(json.dumps(payload).encode("utf-8"))
