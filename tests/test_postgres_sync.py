...
def test_build_sync_plan_adds_unique_suffix_to_temporary_names(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = SyncCallRecorder()
    service = _sync_service(calls)
    values = iter(["aaaaaa111111", "bbbbbb222222"])
    monkeypatch.setattr(
    ...
    first = service.build_sync_plan(
        name="demo",
        admin_url="postgresql://example/postgres",
        target_database="remote_demo",
        drop_previous=False,
    )
    second = service.build_sync_plan(
        name="demo",
        admin_url="postgresql://example/postgres",
        target_database="remote_demo",
        drop_previous=False,
    ...
    assert first.temporary_database != second.temporary_database
    assert first.temporary_database.endswith("_aaaaaa")
    assert second.temporary_database.endswith("_bbbbbb")


def test_url_for_database_preserves_connection_details() -> None:
    url = postgres_sync_module.url_for_database(
        "postgresql://user:pass@example/source?sslmode=require",
        "target",
    )

    assert url == "postgresql://user:pass@example/target?sslmode=require"
